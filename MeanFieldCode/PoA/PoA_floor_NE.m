...
k_floor = k(index_start : floor_end);
...
B_s = zeros(1, length(k_floor));
piBs = zeros(1, length(k_floor));
piBg = zeros(1, length(k_floor));
ratio = zeros(1, length(k_floor));
...
title('PoA (Floor NE): \rho = ' + string(rho) + ', c = ' + string(c), 'FontSize', 15)
