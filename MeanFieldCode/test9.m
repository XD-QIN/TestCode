...
if rho == 1
    ...
    f1 = (1/lambda).* (((B+1)./(rho.^(B+1)-1) ) + (B) + (1/(1-rho)))+ ...
        (k*lambda^3/c^2) .* ((rho.^x_opt - rho.^(x_opt+1))./(1 - rho.^(x_opt+1))).^2 .* ...
    ...
    f2 = (1/lambda).* (((B+1)./(rho.^(B+1)-1) ) + (B) + (1/(1-rho)))+ ...
        (k*lambda^3/c^2) .* ((rho.^(floor(x_opt)) - rho.^((floor(x_opt))+1))./(1 - rho.^((floor(x_opt))+1))).^2 .* ...
        ((rho.^B - rho.^(B+1))./(1 - rho.^(B+1)));
    x2 = floor(x_opt):ceil(x_opt);
    ...
    f3 = (1/lambda).* (((B+1)./(rho.^(B+1)-1) ) + (B) + (1/(1-rho)))+ ...
        (k*lambda^3/c^2) .* ((rho.^(ceil(x_opt)) - rho.^((ceil(x_opt))+1))./(1 - rho.^((ceil(x_opt))+1))).^2 .* ...
        ((rho.^B - rho.^(B+1))./(1 - rho.^(B+1)));
    x3 = floor(x_opt):ceil(x_opt);
