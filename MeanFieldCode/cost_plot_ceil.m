...
for i =  1 : length(k_ceiling)
    ...
    avgQ(:,i) = totalQ(:,i) / T;
    for n = 1 : N
        Cost(1, i) = Cost(1, i) + avgQ(n, i)/lambda + k_ceiling(i) * (lambda * sum( pi_B(:, i))/ (N *c)).^2 * pi_B(n, i);
    end
    Cost(1, i) = Cost(1, i) / N;
    if lambda ~= mu
    ...
    TheoryCost(1, i) = TheoryAvgQ(1, i)/lambda + k_ceiling(i) * (lambda * N * TheoryPi_B(1, i)/ (N * c))^2 * TheoryPi_B(1, i);
end
...
figure(1)
plot(k_ceiling .*lambda^3/c^2,TheoryCost, 'r-.x','LineWidth',1,'MarkerSize',5)
hold on
plot(k_ceiling .*lambda^3/c^2,Cost, 'b-o','LineWidth',1,'MarkerSize',5)
